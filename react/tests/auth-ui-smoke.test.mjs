...
assert.match(loginPage, /signupNotice/)
assert.match(loginPage, /signupMessage/)
...
assert.match(signupPage, /sendEmailVerification/)
assert.match(signupPage, /verifyEmailVerification/)
...
assert.match(signupPage, /name="profileUploadFile"/)
assert.match(signupPage, /accept="image\/\*"/)
assert.match(signupPage, /프로필 사진 선택/)
assert.match(signupPage, /if \(profileImageFile\) \{[\s\S]*signupData\.append\('profileUploadFile', profileImageFile\)/)
assert.match(signupPage, /name="birthDate"/)
assert.match(signupPage, /inputMode="numeric"/)
assert.match(signupPage, /maxLength=\{6\}/)
assert.match(signupPage, /isEmailVerified/)
assert.match(signupPage, /if \(!isEmailVerified\) return/)
assert.match(signupPage, /disabled=\{isSubmitting \|\| emailVerification\.isSending \|\| emailVerification\.isVerifying\}/)
assert.match(signupPage, /signupData\.append\('birthDate'/)
assert.match(signupPage, /SIGNUP_SUCCESS_MESSAGE/)
assert.match(signupPage, /signupMessage: SIGNUP_SUCCESS_MESSAGE/)
...
assert.match(authApi, /\/api\/auth\/email\/send/)
assert.match(authApi, /\/api\/auth\/email\/verify/)
