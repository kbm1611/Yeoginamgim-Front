...
import { AlignCenter, AlignLeft, AlignRight, Camera, Image as ImageIcon, PenLine, Plus, Smile, Trash2, Type, X } from 'lucide-react'
...
// 포스트잇: 3:4 비율, 질감PNG + 배경색 + 그림자 구조
const POSTIT_SIZE = { width: 600, height: 800 }
const POSTIT_EXPORT_ASPECT_RATIO = POSTIT_SIZE.width / POSTIT_SIZE.height
...
const POSTIT_TEMPLATES = [
  ...
  { id: 'pink', label: '핑크', color: '#F6ABBE', tape: '#EFA6B8' },
  { id: 'sky', label: '하늘', color: '#A8D8F0', tape: '#9EC8E8' },
  { id: 'green', label: '연두', color: '#B8E0A0', tape: '#AECF80' },
  { id: 'cream', label: '크림', color: '#FFF0CC', tape: '#D2D97E' },
  { id: 'purple', label: '보라', color: '#D4B8F0', tape: '#C3A2E5' },
]

// textBounds / drawBounds: 카드 표시 크기(810x959) 기준 0~1
// 포스트잇 본체 안쪽 여백 8%(좌우), 10%(상하)
const POSTIT_TEXT_BOUNDS = {
  x: 0.08,
  y: 0.10,
  width: 0.84,
  height: 0.80,
}
const POSTIT_DRAW_BOUNDS = {
  x: 0.04,
  y: 0.05,
  width: 0.92,
  height: 0.90,
...
const CARD_TEMPLATE = {
  postit: {
    size: POSTIT_SIZE,
    textBounds: POSTIT_TEXT_BOUNDS,
    drawBounds: POSTIT_DRAW_BOUNDS,
    excludedZones: [],
  },
  polaroid: {
    size: POLAROID_SIZE,
...
function AddDock({ step, onOpenPicker, onBackToEmpty }) {
  ...
  if (step === EDITOR_STEP.EMPTY) {
    return (
      <div className="absolute bottom-6 left-0 right-0 z-40 flex justify-center">
        <button
          type="button"
          onClick={onOpenPicker}
          className="flex h-14 items-center justify-center gap-2 rounded-full bg-[#3B2418] px-8 text-[15px] font-extrabold text-white shadow-[0_8px_22px_rgba(59,36,24,0.32)]"
        >
...
function remapStrokePoints(points, bounds) {
    ...
    <div className="pointer-events-none absolute z-[4] overflow-hidden" style={boundsToStyle(bounds)}>
      ...
      >
        {strokes.map((stroke) => (
          <path
            key={stroke.id}
            d={outlineToPath(getStrokeOutline(remapStrokePoints(stroke.points, bounds)))}
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden',
        width: cardType === 'polaroid' ? 'min(82vw, 300px)' : 'min(72vw, 260px)',
        borderRadius: 4,
        boxShadow: cardType === 'postit'
          ? '2px 3px 0px rgba(0,0,0,0.06), 4px 8px 18px rgba(0,0,0,0.13), 0px 1px 3px rgba(0,0,0,0.05)'
          : 'none',
      }}
    >
      {cardType === 'postit' ? (
        <>
          ...
          <div
            ...
            style={{ backgroundColor: postitTemplate?.color ?? '#F7E58A' }}
          />
          {/* 질감 */}
          <img
            src={postitTexture}
            alt=""
            draggable={false}
            className="pointer-events-none absolute inset-0 z-[1] h-full w-full rounded-[4px]"
            style={{ mixBlendMode: 'overlay', opacity: 0.9, objectFit: 'cover' }}
          />
        </>
      ) : (
        <>
...
function PostItEditor() {
  ...
  const pendingPhotoBlobRef = useRef(null)
  ...
  useEffect(() => {
    const blobRef = pendingPhotoBlobRef
    return () => {
  ...
  const handlePhotoChange = (event) => {
    ...
    pendingPhotoRef.current = false
    if (pendingPhotoBlobRef.current) {
      URL.revokeObjectURL(pendingPhotoBlobRef.current)
    }
    const blobUrl = URL.createObjectURL(file)
    pendingPhotoBlobRef.current = blobUrl
    setCardType('polaroid')
    setPendingPhoto(blobUrl)
    setSelectedPhoto(null)
    setPhotoCrop({ x: 0.5, y: 0.5, scale: 1 })
  ...
  const cancelPhotoCrop = () => {
    setPendingPhoto(null)
    setSelectedPhoto(null)
    setStep(selectedPhoto ? EDITOR_STEP.EDITING : EDITOR_STEP.TYPE_PICKER)
    if (!selectedPhoto) setCardType(null)
    setPhotoCrop({ x: 0.5, y: 0.5, scale: 1 })
  }
  ...
  const exportImage = async () => {
    ...
    if (cardType === 'postit') {
      // 배경색
      ctx.fillStyle = postitTemplate?.color ?? '#F7E58A'
      ctx.fillRect(0, 0, W, H)
      // 질감 multiply
      const paper = await loadImage(postitTexture)
      ctx.save()
    ...
    objects.forEach((object) => {
      if (object.type === OBJECT_TYPE.STROKE) {
        const db = activeTemplate.drawBounds
        const drawRect = { x: db.x * W, y: db.y * H, width: db.width * W, height: db.height * H }
        const outline = getStroke(
          object.points.map((point) => [
            ((point.x - db.x) / db.width) * drawRect.width + drawRect.x,
            ((point.y - db.y) / db.height) * drawRect.height + drawRect.y,
            point.pressure ?? 0.5,
          ]),
          {
            ...FREEHAND_OPTIONS,
      ...
      const x = object.x * W
      const y = object.y * H
      const width = (object.width ?? 0.1) * W
      const height = (object.height ?? 0.1) * H
      ...
      if (object.type === OBJECT_TYPE.STICKER) {
        ...
        ctx.fillText(object.value, x + width / 2, y + height / 2)
      } else {
